 ...
 * @return {Promise<import("child_process").ChildProcess>}         reference to the created child process
 */
  async start(file,opts={}) {
    ...
    if ( res.is_dbus ) {
      ...
      this.logger.debug(`Opening ${file} with executable ${entry["Exec"]}`);
      try{
   ...
   * @param {import("child_process").SpawnOptions} [opts] - Options to be passed to spawn's third arg
   */
