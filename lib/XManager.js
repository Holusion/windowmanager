...
XManager.prototype.init = function () {
  ...
  x11.createClient(function(err,display){
    ...
    self.X.ChangeWindowAttributes(self.root, { eventMask: x11.eventMask.Exposure|x11.eventMask.SubstructureRedirect }, function(err) {
        if(err.error == 10){
      ...
      if(ev.name == "KeyPress"){
        var keys = self.keyboard.getKey(ev);
        if(keys){
          self.emit("KeyPress",modifiers+key);
        }
