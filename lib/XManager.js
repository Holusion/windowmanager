...
class XManager extends EventEmitter{
  ...
  get isExposed(){ //Always return true if no display
    return !this.isActive || (1 == Object.keys(this.frames).length);
  }
	get isActive(){
  ...
  init(callback) { //callback is optionnal
    ...
    callback = (typeof callback == "function")? callback: function(err){
      ...
      self.X.ChangeWindowAttributes(rid, { eventMask: x11.eventMask.Exposure|x11.eventMask.SubstructureRedirect }, function(err) {
        if(err.error == 10){
    ...
    if(process.env["NODE_ENV"] == "development"){
      ...
      switch(ev.type){
        ...
        case 17: //DestroyNotify
          ...
          if(Object.keys(self.frames).length ==1){
            self.emit("expose",ev);
          }else{
