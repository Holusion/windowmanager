var util = require("util")
  ...
  , XBackground = require("./XBackground");
      ...
      switch(ev.type){
        ...
        case 17: //DestroyNotify
          ...
          if(Object.keys(self.frames).length ==0 && self.bg){
            console.log("set focus : ",self.bg.win);
            self.X.SetInputFocus(self.bg.win,1);
            self.X.RaiseWindow(self.bg.win);
  ...
  self.X.GetGeometry(self.root, function(err, clientGeom) {
    ...
    self.X.require('render', function(error, Render) {
      ...
      xpm.open(__dirname+"/../data/logo.xpm",function(err,logo){
        ...
        this.bg = new XBackground(logo,wid,self.X,Render);
      });
    });
  });
};

XManager.prototype.exit = function(){
  if(this.bg){
