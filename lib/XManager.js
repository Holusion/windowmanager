    ...
    this.X.on('error', function(err) {
      ...
      switch(ev.type){
        ...
        case 23: //ConfigureRequest
          console.log(`MoveResize Window ${ev.wid} to ${ev.width}x${ev.height}${Math.sign(ev.x) == -1 ? "-":"+"}${ev.x}${Math.sign(ev.y) == -1 ? "-":"+"}${ev.y}`)
          this.X.MoveResizeWindow(ev.wid, ev.x, ev.y, ev.width, ev.height);
          break;
