...
class XManager extends EventEmitter{
  constructor(display, {autoResize= false, logger=Logger.logger}={}){
    super();
    ...
    this.X.on('error', (err)=>{
      this.logger.warn("WindowManager : ",err);
      /*
      ...
      switch(ev.name){
        ...
        case "DestroyNotify": //DestroyNotify
          ...
          this.logger.info("received a destroyNotify for : ",ev.wid);
          ...
          if(Object.keys(this.frames).length ==0){
            ...
            this.logger.info("still managing frames : ", this.frames);
          }
          break;
        case "UnmapNotify":
        case "MapNotify":
        ...
        case "ConfigureNotify":
          this.logger.debug("Configure Notify : ",ev);
          this.X.GetProperty(0, ev.wid, this.X.atoms.WM_NORMAL_HINTS, this.X.atoms.WM_SIZE_HINTS, 0, 1000, (err, prop)=>{
            if(err) return this.logger.log("GetProperty (ConfigureRequest)", err);
            ...
            this.logger.info("Ignoring configure Notify data with flags : ", flag_names.join(", "));
          })
          break;
        case "ConfigureRequest":
          this.logger.debug(`ConfigureRequest ${ev.wid} to ${ev.width}x${ev.height}${Math.sign(ev.x) == -1 ? "-":"+"}${ev.x}${Math.sign(ev.y) == -1 ? "-":"+"}${ev.y}`)
  ...
  handleClientMessage(ev){
    this.X.GetAtomName(ev.message_type, (err, name)=>{
      ...
      switch(name){
        case '_NET_WM_STATE':
          this.logger.info("WM_STATE : ", ev.data.map(d=>this.X.atom_names[d] || d))
          break;
    ...
    this.X.GetWindowAttributes(wid, (err, attrs) => {
      ...
      .then((clientGeom)=>{
        ...
        if(this.autoResize){
          ...
          this.logger.info(`Window root geometry: ${this.width}x${this.height}`);
          this.logger.info(`Trying to resize to : ${pos.width}x${pos.height}`);
      ...
      .then((pos)=>{
        ...
        ee.on('event', (ev)=> {            
          ...
          {
            this.logger.info("destroy : ",fid);
            this.X.DestroyWindow(fid);
            delete this.frames[fid];
          }else{
            this.logger.info("FID event : ", ev.name);
          }
        });
      })
    });
  ...
  createBackground () {
    ...
    this.X.GetGeometry(this.root, (err, clientGeom)=>{
      ...
      this.X.require('render', (error, Render)=>{
        ...
        }).catch(function(e){this.logger.error("cannot load background XPM : ",e)});
      });
    });
  }
