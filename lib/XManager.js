...
class XManager extends EventEmitter{
  constructor(display, opts={autoResize: false}){
    ...
    this.xtestExt = new Promise((resolve)=>{
        this.X.require("xtest", function(err, ext){
          if(err){
            console.warn("failed to require extension xtest");
            resolve();
          }else{
            resolve(ext);
          }
    ...
    console.log(`Initial screen size : ${this.width}x${this.height}`);

    this.X.require("randr", (err, Randr)=>{
      if(err){
        return this.emit("error", err);
      }
      console.log("Randr signature : ", Randr);
    ...
    .then(async ([wm_state_atom, wm_fullscreen_atom])=>{
      console.log("Type : ", this.X.atoms.ATOM);
      ...
      switch(ev.name){
        ...
        case "DestroyNotify": //DestroyNotify
          ...
          if(Object.keys(this.frames).length ==0){
            ...
            console.log("still managing frames : ", this.frames);
          }
          break;
        case "UnmapNotify":
        case "MapNotify":
        ...
        case "ConfigureNotify":
          ...
          this.X.GetProperty(0, ev.wid, this.X.atoms.WM_NORMAL_HINTS, this.X.atoms.WM_SIZE_HINTS, 0, 1000, (err, prop)=>{
            ...
            console.log("Ignoring configure Notify data with flags : ", flag_names.join(", "));
          })
          break;
        case "ConfigureRequest":
          console.log(`ConfigureRequest ${ev.wid} to ${ev.width}x${ev.height}${Math.sign(ev.x) == -1 ? "-":"+"}${ev.x}${Math.sign(ev.y) == -1 ? "-":"+"}${ev.y}`)
          ...
          break;
        case "PropertyNotify":
          console.log("Property change for atom ",this.X.atom_names[ev.atom] || ev.atom);
          break;
        case "ClientMessage":
        ...
        case "RRScreenChangeNotify":
          //console.log(`RandR screen change : ${ev.width}x${ev.height}`);
          this.width = ev.width;
        ...
        default:
          if(ev.type !=21){ //ReparentNotify has no name  assigned for whatever reason
            console.log("Uncatched win event : ",ev.name? ev.name : ev.type);
          }
      }
    });
  }
  ...
  handleClientMessage(ev){
    this.X.GetAtomName(ev.message_type, (err, name)=>{
      if(err) return console.warn("Failed to get ATOM :", err);
      switch(name){
        case '_NET_WM_STATE':
          console.log("WM_STATE : ", ev.data.map(d=>this.X.atom_names[d] || d))
          break;
        case '_NET_REQUEST_FRAME_EXTENTS':
          break;
        default:
          console.log("Unknown ATOM : ", name, ev);
      }
    })
  }
  ...
  manage(wid){
    this.X.GetWindowAttributes(wid, (err, attrs) => {
      //console.log("Window Attributes : ", attrs);
      if (attrs[8]){ // override-redirect flag
        ...
        //console.log("Manage window with attributes : ", attrs);
      }
      ...
      .then((clientGeom)=>{
        ...
        if(this.opts.autoResize){
          ...
          console.log(`Root geometry: ${this.width}x${this.height}`);
          console.log(`Try to resize window to : ${pos.width}x${pos.height}`);
