...
XManager.prototype.init = function () {
  ...
  x11.createClient(function(err,display){
    if(err){
      throw err; //We should know about any error at this point as it's critical.
    }
...
XManager.prototype.focus = function(){
  if(this.bg){
    this.bg.focus = true;
  ...
  self.X.GetGeometry(self.root, function(err, clientGeom) {
    ...
    self.X.require('render', function(error, Render) {
      ...
      loadXpm(__dirname+"/../data/logo.xpm").then(function(logo){
        self.bg = new XBackground(logo,wid,self.X,Render);
